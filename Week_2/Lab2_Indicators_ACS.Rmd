...
C <-
  ...
  scale_fill_manual(values = palette5,  
                    labels = qBr(totalPop16, "estimate"), #this is the function defined by the class instructor
                    name = "Total\nPopluation\n(Quintile Breaks)") +
  theme(plot.title = element_text(size=22)) #this is defined by fich too 
...
septaBuffers <- 
  rbind(
    st_buffer(septaStops, 2640) %>%
      ...
      dplyr::select(Legend),
    st_union(st_buffer(septaStops, 2640)) %>% #the number at the end is in feet, because our CRS is in feet
      st_sf() %>%
...
```{r}
allTracts.Summary %>%
  gather(Variable, Value, -year, -TOD) %>% #pivot wide and pivot longer are other functions that do this, pivot longer is gather, pivot wide you'll have to check online, not explained in class
...
```{r}
centerCity <- #the places where the buffers intersect is beign called CC
  st_intersection( 

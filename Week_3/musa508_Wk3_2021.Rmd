...
plot(lancCounty) #for every field
```
...
#for some reason when we dont do stdifference the actual area gets included 
...
#my_palette <- viridis_pal(option = "D")(length(unique(buffersAndTowns$MUNI)))



#ggplot() +
 # geom_sf(data = buffersAndTowns, aes(fill = MUNI)) +
 # geom_sf_text(data = studyAreaTowns, aes(label = MUNI), size = 2) +
 # scale_fill_manual(values = my_palette) + 
 # labs(title = "Buffers and Municipalities")+
 # mapTheme()


ggplot() +
  geom_sf(data = uga) +
  geom_sf(data = buffersAndTowns %>% filter(Legend == "Inside"), aes(fill = MUNI), alpha = 0.4) +
  geom_sf(data = buffersAndTowns %>% filter(Legend == "Outside"), aes(fill = MUNI), alpha = 0.9) +
  geom_sf_text(data =buffersAndTowns %>% filter(Legend == "Inside"), check_overlap = TRUE, aes(label = MUNI), size = 3) +
